...
class UserController extends Controller
{
    ...
    public function update(Request $r, $id)
    {
        ...
        if ($user->id === $r->user()->id && ($data['role'] !== 'admin' || !$data['is_active'])) {
            return back()->withErrors(['role' => 'You cannot remove or deactivate your own administrator access.'])->withInput();
        }

        if ($user->isAdmin() && ($data['role'] !== 'admin' || !$data['is_active']) && User::where('role', 'admin')->where('is_active', true)->count() <= 1) {
            return back()->withErrors(['role' => 'At least one active administrator must remain.'])->withInput();
        }
